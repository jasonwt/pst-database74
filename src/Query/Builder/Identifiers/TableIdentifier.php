...
class TableIdentifier extends CoreObject implements ITableIdentifier {
    ...
    public static function tryParse(string $input): ?self {
        $pregPattern = PregPatterns::TABLE_IDENTIFIER_WITH_OPTIONAL_ALIAS_PATTERN;

        if (!preg_match("/^$pregPattern$/i", $input, $matches)) {
            return null;
        }
    ...
    public static function new(string $tableName = null, ?string $schemaName = null, ?string $alias = null): ITableIdentifier {
